...
//https://juejin.im/post/5a9e57af6fb9a028df222555#heading-17
...
- (void)useBlockOperation {
    ...
    NSBlockOperation *op = [NSBlockOperation blockOperationWithBlock:^{
        for (int i = 0; i < 2; i++) {
            [NSThread sleepForTimeInterval:2]; // 模拟耗时操作
            NSLog(@"1-useBlockOperation--%@", [NSThread currentThread]); // 打印当前线程
        }
    }];
...
- (void)useCustomOperation {
    ...
    YJCustomOperation *op = [[YJCustomOperation alloc] init];
    // 2.调用 start 方法开始执行操作
    [op start];
}
